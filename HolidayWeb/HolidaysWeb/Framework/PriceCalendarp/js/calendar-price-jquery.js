...
(function ($) {
    ...
    var createSettingWindow = function () {
        if ($(".capricorncd-date-detailed-settings").length > 0) {
            return $(".capricorncd-date-detailed-settings");
        }
    ...
    fn.createCalendar = function () {
        ...
        if (!this.opts.hideFooterButton) {
            ...
            html += '        <button class="btn btn-confirm " class="btn " data-dismiss="modal">确定</button>';
            html += '        <button class="btn btn-cancel btn btn-default"  type="button"  data-dismiss="modal">取消</button>';
            html += '    </div>';
        }
        html += '</div>';
    ...
    fn.handleClickEvent = function () {
        ...
        this.calendar.on('click', '.btn-confirm', function () {
            me.opts.callback(me.opts.newData);
        });
        ...
        this.settingWindow.on('click', '.btn-confirm', function () {
            ...
            var key = me.opts.isRealNum(isRealNum(setData.price));
            if (!key) {
                return;
            }
    ...
    fn._updateThisData = function (setData, dateString) {
        ...
        data["ShopId"] = this.opts.ShopId;
        this.opts.newData.push(data);
        if (!is_existence) {
            this.data.push(data);
        }
